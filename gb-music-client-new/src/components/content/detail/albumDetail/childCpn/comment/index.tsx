...
const Comment: FC<IProps> = (props): ReactElement => {
  ...
  useEffect(() => {
    getAllComment<{ comments: IComment[]; count: number }>(id, 'alId', 0, 10).then((data) => {
  ...
  const handlePageChange=(e:number)=>{
    getAllComment<{ comments: IComment[]; count: number }>(id, 'alId', (e-1)*10, 10).then((data) => {
      setComment(data.comments);
      setTotal(data.count);
    });
  }
  return (
    <CommentWrapper>
      ...
      <p>专辑评论</p>
      <AlbumComment pageClick={(e)=>handlePageChange(e)} comments={comment} onClick={() => reply()} total={total} isPage={true} />
    </CommentWrapper>
  );
};
