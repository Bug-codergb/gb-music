...
import { getVideoDataApi } from '@/api/modules/video';
export default {
  name: 'Video',
  components: { Header },
  data() {
  ...
  methods: {
    async initVideoData() {
      const result = await getVideoDataApi();
      ...
      this.videoEcharts = videoEcharts
    ...
    handleResize(){
      ...
      this.videoEcharts && this.videoEcharts.resize();
    }
  }
};
</script>
