...
const Comment: FC<IProps> = ({ id }): ReactElement => {
  ...
  //获取所有评论
  useEffect(() => {
    getAllComment(id, 'pId', 0, 10).then((data: any) => {
      //console.log(data);
  ...
  const handlePageChange=(e:number)=>{
    getAllComment(id, 'pId', (e-1)*10, 10).then((data: any) => {
      setComment(data.comments);
      setTotal(data.count);
    });
  }
  return (
    <CommentWrapper>
      <div className="publish-comment">
        ...
        <p>歌单评论:</p>
        <PlayListComment comments={comment} pageClick={(e)=>handlePageChange(e)} onClick={() => reply()} isPage={true} total={total} />
      </div>
    </CommentWrapper>
  );
};
