...
import { getChannelDataApi } from '@/api/modules/channel';
export default {
  name: 'Channel',
  components: { Header },
  data() {
  ...
  async mounted() {
    const result = await getChannelDataApi();
    ...
    this.channelEcharts = channelEcharts;
  ...
  methods:{
    handleResize(){
      this.channelEcharts && this.channelEcharts.resize();
    }
  }
};
</script>
